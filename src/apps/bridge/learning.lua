...
--
--   pft_size (default 512)
--
--     Size of each of the three packet forwarding tables (unicast,
--     flooding, discard), see performance notes below.  A warning
--     message is logged if this default needs to be changed for a
...
function bridge:new (arg)
   ...
   o._pft_size = o._conf.config.pft_size or default.pft_size
   ...
   for type, alloc_pl in pairs(pft_spec) do
      pft[type] = ffi.new(pft_t, o._pft_size)
      if alloc_pl then
         for i = 0, o._pft_size-1 do
      ...
      o._pft_C[i] = ffi.cast("pft_t*", pft[type])
      i = i+1
   end
   o._pft = pft
   -- Box to store a pointer to a MAC address in memory
   o._mac = ffi.new("uint8_t *[1]")
   o._logger = lib.logger_new({ rate = 0.1, module = "bridge" })
   return o
end
   ...
   while ports[ip] do
      ...
      while not empty(l_in) do
         ...
         if packets >= pft_size then
            -- This doesn't necessarily mean that any of the tables is
            -- actually full, but we err on the side of caution.
            if self._logger:can_log() then
               self._logger:log("packet forwarding table exhausted "
                                   .."(size "..pft_size.."), "
                                   .."please increase pft_size configuration.")
            end
            goto BREAK
         end
      end
